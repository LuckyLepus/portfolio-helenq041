...
import { Video, AlertCircle, Power, ShieldCheck } from 'lucide-react';

const TrackingOverlay = ({ onExit }: { onExit: () => void }) => {
  const { isTracking, headMovement, handPush, error, videoRef, stopTracking } = useFaceTracking();
    ...
    <>
          ...
          <div className="border border-red-500/50 bg-red-950/40 p-8 max-w-lg text-center flex flex-col gap-4 shadow-[0_0_50px_rgba(255,0,0,0.1)]">
            ...
            <div className="mt-4 flex justify-center gap-3">
              <button
                ...
                className="px-6 py-2 border border-white/20 text-white/70 transition-colors uppercase tracking-widest text-xs hover:bg-white/10"
              >
              ...
              <button
                ...
                className="px-6 py-2 bg-red-900/40 hover:bg-red-800/60 border border-red-500/50 text-red-100 transition-colors uppercase tracking-widest text-xs"
              >
                Reload Interface
              </button>
            </div>
      ...
      <div className="absolute inset-0 p-8 flex flex-col justify-between pointer-events-none z-30 font-mono text-[#e0e0e0]">
        <button
          ...
          className="pointer-events-auto absolute top-8 right-8 flex items-center gap-2 border border-red-500/40 bg-black/70 px-3 py-2 text-[10px] uppercase tracking-widest text-red-300 hover:bg-red-950/70"
        >
        ...
        <div className="flex justify-between items-start">
          ...
          <div className="mr-36 text-right text-xl tracking-tighter whitespace-pre-wrap leading-tight">
            {timeStr}
          </div>
        </div>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const stopWhenHidden = () => {
      if (document.visibilityState === 'hidden') setStarted(false);
    };
      ...
      <div className="flex h-screen w-screen bg-[#0a0a0a] text-[#c2ae6a]/70 font-mono items-center justify-center relative overflow-hidden">
        ...
        <div className="flex flex-col items-center justify-center p-8 max-w-md w-full bg-[#1a1a1a] border border-[#333] z-10 text-center gap-6 shadow-[0_0_50px_rgba(0,0,0,1)]">
          ...
          <div className="text-sm text-[#c2ae6a]/70 border-y border-[#333] py-4 leading-relaxed text-left flex flex-col gap-4">
            <p>
              This terminal connects to an off-axis viewing portal.
            </p>
            <div className="flex gap-3 border border-green-800/50 bg-green-950/20 p-3 text-[11px] text-green-200/80">
              <ShieldCheck className="mt-0.5 h-4 w-4 shrink-0" />
              <span>
                Camera frames and landmarks are processed locally in this tab. Recognition model
                files load from a public CDN, but camera images are not sent there. Nothing is
                uploaded or saved. Switching tabs or pressing Stop Camera ends the session.
              </span>
            </div>
          ...
          >
            ...
            I understand — start local camera
          </button>
        </div>
      </div>
    );
    ...
    <div className="w-screen h-screen bg-[#0a0a0a] overflow-hidden flex items-center justify-center font-mono text-[#e0e0e0] relative select-none">
      ...
      <div className="relative w-full max-w-[1024px] aspect-[4/3] max-h-[85vh] border-4 border-[#333] shadow-[0_0_50px_rgba(0,0,0,1)] bg-[#1a1a1a] overflow-hidden m-4">
        <FaceTrackingProvider>
          <TrackingOverlay onExit={() => setStarted(false)} />
